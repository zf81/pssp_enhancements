...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqldb://' + mysql_username + ':' + mysql_password + '@' + mysql_host + ':3306/patient_portal_assignment'
...
def get_gui_patients():
    ...
    if 'loggedin' in session and session['account_type'] == 'careprovider':
...
# update care provider information
@app.route('/edit_careprovider', methods = ['GET', 'POST'])
def edit_careprovider(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ## get id from form
        form_id = request.form.get('id')
        provider = Users.query.filter_by(id=form_id).first()
        provider.username = request.form.get('username')
        provider.email = request.form.get('email')
        db.session.commit()
        flash("Care Provider Information Updated Successfully")
        return redirect(url_for('get_gui_patients'))
